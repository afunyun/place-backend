document.addEventListener("DOMContentLoaded", () => {
	...
	function findClosestPaletteColor(hexColor) {
		// Convert hex to RGB
		const targetR = Number.parseInt(hexColor.slice(1, 3), 16);
		const targetG = Number.parseInt(hexColor.slice(3, 5), 16);
		const targetB = Number.parseInt(hexColor.slice(5, 7), 16);

		let closestColor = PALETTE[0];
		let minDistance = Number.POSITIVE_INFINITY;

		for (const paletteColor of PALETTE) {
			const r = Number.parseInt(paletteColor.slice(1, 3), 16);
			const g = Number.parseInt(paletteColor.slice(3, 5), 16);
			const b = Number.parseInt(paletteColor.slice(5, 7), 16);

			// Calculate Euclidean distance in RGB space
			const distance = Math.sqrt(
				(targetR - r) ** 2 + (targetG - g) ** 2 + (targetB - b) ** 2,
			);
