...
async function loadBackupGrid() {
  ...
  try {
    ...
    console.log(
      `🚀 Uploading ${pixelCount} pixels in ${totalBatches} batches...`,
    );
    ...
    for (let i = 0; i < updates.length; i += batchSize) {
      ...
      console.log(
        `🔄 Batch ${batchNum}/${totalBatches} (${batch.length} pixels)...`,
      );

      const promises = batch.map(async (update) => {
        try {
          const response = await fetch(
            'https://place-worker.afunyun.workers.dev/pixel',
            {
              method: 'POST',
              headers: {
          ...
          failCount++;
          console.log(
            `   ❌ Error: (${update.x},${update.y}) - ${error.message}`,
          );
    ...
    if (failCount > 0) {
      console.log(
        '⚠️ Some pixels failed to load. The Worker should still be functional.',
      );
    } else {
